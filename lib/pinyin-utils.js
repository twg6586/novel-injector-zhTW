...
// Boundary table adapted from tiny-pinyin / Android HanziToPinyin (MIT-style upstream).
...
// 將 pinyin-utils.js 的 niHanziToPinyin 改為以下代碼，放棄拼音，改用安全的 URI 編碼或保留英文
export function niHanziToPinyin(ch) {
    if (!ch) return '';
    // 如果是英數字，直接保留
    if (/^[a-zA-Z0-9]$/.test(ch)) return ch.toLowerCase();
    // 否則轉為 URI 編碼並去掉 % 以符合檔名規範 (或直接回傳 'x' 等占位符，因為檔名後方有隨機 key)
